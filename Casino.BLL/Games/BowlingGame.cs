...
public class BowlingGame : Game
{
    ...
    {
        var hitResult = await _telegramBotClient.SendDiceAsync(_gameModel.Chat.Id, Emoji.Bowling);
    ...
    protected override async Task SendRoundResultMessageAsync(SaveGameResultModel saveGameResultModel)
    {
        ...
        if (saveGameResultModel.Success)
        {
            roundResultMessage = _gameModel.BettingResult.IsWon
                ? _localizer[Resources.WonMessageResource, (int)WinningsScore, _gameModel.IsDemoPlay ? AppConstants.DemoCurrencySign : AppConstants.RealCurrencySign]
